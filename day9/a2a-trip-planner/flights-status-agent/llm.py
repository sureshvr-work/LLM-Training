...
# ===========================================================================
# STREAMING TURN  —  same SENSE -> REASON, but ACT *streams* its updates out.
# This is the message/stream shape: the model still just picks the tool once;
# the agent then runs a tool that yields a sequence of updates over time, and
# we surface each one as it happens (the caller wraps them as SSE frames).
# ===========================================================================
async def run_stream(system: str, user_text: str, tool: dict, stream_tool):
    """Async-generator turn. Yields dicts:
       {"type":"open","trace":[...],"input":{...}}  once, carrying the reasoning
       {"type":"update","update":{...}}             for every streamed tool update
       {"type":"text","text":...}                   if the model answered w/o a tool
    """
    trace = []
    context = sense(system, user_text, tool, [], trace)          # SENSE
    decision = await reason(context, allow_tool=True, trace=trace)  # REASON
    if decision["type"] != "tool_use":
        yield {"type": "text", "text": decision["text"], "trace": trace}
        return

    # ACT — the agent runs the tool, but this tool STREAMS (yields over time)
    trace.append({"phase": "ACT", "kind": "tool", "label": "ACT \u00b7 streaming the tool (SSE)",
                  "detail": f'{decision["name"]}({json.dumps(decision["input"])}) \u2192 live updates'})
    yield {"type": "open", "trace": trace, "input": decision["input"]}
    async for update in stream_tool(decision["name"], decision["input"]):
        yield {"type": "update", "update": update}
